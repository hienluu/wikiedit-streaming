...
import org.apache.spark.sql.execution.streaming.{HDFSMetadataLog, LongOffset, Offset, SerializedOffset, Source}
 ...
 */
class MyRateSourceProvider extends StreamSourceProvider with DataSourceRegister {
  ...
  override def sourceSchema(
    ...
    (shortName(), MyRateSourceProvider.SCHEMA)
  ...
  override def shortName(): String = "my_rate"
}

object MyRateSourceProvider {
  val SCHEMA =
...
class MyRateStreamSource(
  ...
  import MyRateSourceProvider._
  import MyRateStreamSource._

  log.warn("**** In the constructor of MyRateStreamSource")
 ...
 // val clock = if (useManualClock) new ManualClock else new SystemClock
  ...
  override def schema: StructType = MyRateSourceProvider.SCHEMA

  override def getOffset: Option[Offset] = {

    val now:Long = clock.getTimeMillis()
    if (lastTimeMs < now) {
      lastTimeMs = now
    }
  ...
  override def getBatch(start: Option[Offset], end: Offset): DataFrame = {
    ...
    if (rangeStart == rangeEnd) {
      return sqlContext.createDataFrame(sqlContext.sparkContext.emptyRDD[Row], schema)
    ...
    val rdd = sqlContext.sparkContext.range(rangeStart, rangeEnd, 1, numPartitions).map { v =>
      ...
      Row(new Timestamp(relative + localStartTimeMs), v)
    }
...
private[org] class MySystemClock  {
